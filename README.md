...
> [!WARNING]
> **Minimum distance alone can make visit tracking worse, not better.** Home Assistant device trackers already update infrequently while stationary (see [Known Issues](#known-issues) below), and this setting only suppresses updates further. If you enable it, do so together with something that guarantees a point still reaches Dawarich periodically while stationary — for example a `time_pattern` automation that calls a location-push mechanism on a schedule. If your Dawarich server has its (opt-in, off by default) stay-point visit detector enabled — `Flipper.enable(:stay_point_detection)`, or toggled at `/admin/flipper` — it exposes a **"Visit Max Gap" slider in the Map v2 settings panel** (`stay_max_gap_minutes`, 5-720 minutes, default 60): keep your fallback schedule shorter than that value so a stationary period still gets recognized as a single continuous visit instead of being split into several. Using Minimum distance with no such fallback in place reintroduces the exact gap problem it doesn't solve on its own.
>
